...
// Store current hide preferences
let hidePrefs = { positive: false, neutral: false, negative: false };
...
function applySentimentToElement(articleEl, labelScores) {
  ...
  // Check if this post should be hidden based on preferences
  updatePostVisibility(articleEl, label);
}

function updatePostVisibility(articleEl, label) {
  const shouldHide = hidePrefs[label] === true;
  if (shouldHide) {
    articleEl.classList.add('ps-post-hidden');
  } else {
    articleEl.classList.remove('ps-post-hidden');
  }
}
...
// Load hide preferences from storage
function loadHidePreferences() {
  chrome.storage.local.get([HIDE_PREFS_KEY], (result) => {
    hidePrefs = result[HIDE_PREFS_KEY] || { positive: false, neutral: false, negative: false };
    // Reapply visibility based on loaded preferences
    applyVisibilityToAllPosts();
  });
}

// Apply visibility to all currently visible posts
function applyVisibilityToAllPosts() {
  const posts = document.querySelectorAll(POST_SELECTOR);
  posts.forEach((post) => {
    const label = post.getAttribute('data-ps-sentiment-label');
    if (label) {
      updatePostVisibility(post, label);
    }
  });
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'updateHidePrefs') {
    hidePrefs = message.prefs;
    applyVisibilityToAllPosts();
  }
});
